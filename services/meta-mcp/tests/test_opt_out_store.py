...
def test_load_tenant_config_opt_out_reflects_store(tmp_path: Path) -> None:
    """The store value for opt_out is the authoritative source of truth."""
    store = TenantOptOutStore(tmp_path / "opt_out")
    _run(store.set_opt_out(TENANT_A, True))

    # Should reflect the store's True.
    cfg = _run(load_tenant_config(TENANT_A, store))
    assert cfg.opt_out is True
