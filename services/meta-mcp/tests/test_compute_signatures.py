...
def test_compute_ontology_shape_branching_factor_above_one_preserved(tenant_config):
    ...
    The old implementation mistakenly applied _clamp01, silently discarding
    signal. This test pins the correct behaviour end-to-end through the
    collector function (not just the downstream checker).
    """
    ...
    out = compute_ontology_shape(raw, tenant_config)
    # Median (p50) of [1.5,2.0,2.5,3.0,3.5,4.5,7.0] sorted = 3.0.
    # p95 of 7 values at idx=6 → 7.0.  Both must come through > 1.
