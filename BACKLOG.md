...
**Exhausted.** All previously-listed overnight-safe tickets are Done as of `d906ce9` (2026-05-24, M1-QA-03). The next overnight fire should detect this and stop cleanly with a `notes/orchestrator.md` entry. Candidates to consider for re-stocking (interactively, with Josh's blessing): `M1-ING-06` (re-indexing scheduler — pure ingestion-side, no external services), or a low-risk subset of the CS-02 native tool-use refactor.
