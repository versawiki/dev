...
- `OPS-02 — Apple Developer + Google Play accounts` — Per `docs/operations/app-store-prep.md`. **Decision 2026-05-26: wait for LLC active (~May 29), then sign up both as Versawiki LLC organization** (skip individual-then-convert path; Apple conversion is 2-4 weeks). Pre-signup TODOs: (1) request D-U-N-S number from D&B (https://www.dnb.com/duns-number.html) the day the Sunbiz filing PDF arrives — D-U-N-S takes 2-4 weeks and Apple won't approve org account without it; (2) deploy a placeholder privacy-policy page at https://versawiki.com/privacy (required for Google Play submission); (3) bundle ID convention: `ai.versawiki.app` (Josh owns versawiki.ai; matches both the AI-product framing and owned domain). Total cost on signup day: $124 ($99 Apple + $25 Google).
